...
describe("comparison workbench", () => {
  it("renders the default filtered snapshot", async () => {
    ...
    expect(screen.getByLabelText("当前结果摘要")).toHaveTextContent("16 模型");
    expect(await screen.findByTestId("price-chart")).toHaveTextContent("44 points · USD");
  ...
  it("switches currency while keeping unified subscription valuation", async () => {
    ...
    expect(await screen.findByTestId("price-chart")).toHaveTextContent("44 points · CNY");
  ...
  it("filters estimates and applies a custom Index threshold", async () => {
    ...
    expect(await screen.findByTestId("price-chart")).toHaveTextContent("17 points");
  });
